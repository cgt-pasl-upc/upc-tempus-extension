...
describe('PermisManager', function() {
  ...
  describe('sumatoriPerTipus()', function() {
    it('sumatori d\'array buit', function() {
      var tipus = new TipusPermis('Permis 0', true);
      var permisos = [];
    ...
    it('sumatori amb diferents tipus', function() {
      var tipus1 = new TipusPermis('Permis 0', true);
      var tipus2 = new TipusPermis('Permis 1', false);
      var permisos = [
    ...
    it('sumatori amb diferents tipus i multiples tipus', function() {
      var tipus1 = new TipusPermis('Permis 0', true);
      var tipus2 = new TipusPermis('Permis 1', true);
      var tipus3 = new TipusPermis('Permis 2', false);
      var permisos = [
        new Permis('01/01/2010', new Temps(1), tipus1),
        new Permis('02/01/2010', new Temps(1,30), tipus2),
    ...
    it('sumatori amb codi iguals i noms diferents', function() {
      var tipus1 = new TipusPermis('Permis 0', true);
      var tipus2 = new TipusPermis('Permis 1', true);
      var tipus3 = new TipusPermis('Permis 2', false);
      var permisos = [
      ...
      assert.deepStrictEqual(PermisManager.sumatoriPerTipus([tipus1,tipus3], permisos), new Temps(5));
    });
  ...
  describe('createPermisDiesPerNom()', function() {
    it('permís existent', function() {
      var tipus = new TipusPermis('Permís 01').perDies().recuperable();
      PermisManager.tipusPermisos = [ tipus ];

      var permis = PermisManager.createPermisDiesPerNom('01/01/2020', 'Permís 01');
      assert.deepStrictEqual(permis.data, '01/01/2020');
      assert.deepStrictEqual(permis.temps, new Temps(7, 30));
      assert.deepStrictEqual(permis.tipus, new TipusPermis('Permís 01').perDies().recuperable());
    });
    it('permís no existent', function() {
      PermisManager.tipusPermisos = [];
      var permis = PermisManager.createPermisDiesPerNom('01/02/2020', 'Permís 02');
      assert.deepStrictEqual(permis.data, '01/02/2020');
      assert.deepStrictEqual(permis.temps, new Temps(7, 30));
      assert.deepStrictEqual(permis.tipus, new TipusPermis('Permís 02').perDies().noRecuperable());
    });
  });

  describe('createPermisHoresPerNom()', function() {
    it('permís existent', function() {
      var tipus = new TipusPermis('Permís 01').perHores().recuperable();
      PermisManager.tipusPermisos = [ tipus ];

      var permis = PermisManager.createPermisHoresPerNom('01/01/2020', new Temps(3, 30), 'Permís 01');
      assert.deepStrictEqual(permis.data, '01/01/2020');
      assert.deepStrictEqual(permis.temps, new Temps(3, 30));
      assert.deepStrictEqual(permis.tipus, new TipusPermis('Permís 01').perHores().recuperable());
    });
    it('permís no existent', function() {
      PermisManager.tipusPermisos = [];
      var permis = PermisManager.createPermisHoresPerNom('01/02/2020', new Temps(2, 15), 'Permís 02');
      assert.deepStrictEqual(permis.data, '01/02/2020');
      assert.deepStrictEqual(permis.temps, new Temps(2, 15));
      assert.deepStrictEqual(permis.tipus, new TipusPermis('Permís 02').perHores().noRecuperable());
    });
  });

  describe('getTipusPermisPerNom()', function() {
    it('permisos existents', function() {
      var tipus1 = new TipusPermis('Permís 01').perDies().recuperable();
      var tipus2 = new TipusPermis('Permís 02').perHores().noRecuperable();
      PermisManager.tipusPermisos = [ tipus1, tipus2 ];
      assert.deepStrictEqual(PermisManager.getTipusPermisPerNom('Permís 01'), tipus1);
      assert.deepStrictEqual(PermisManager.getTipusPermisPerNom('Permís 02'), tipus2);
    });
    it('permisos per defecte', function() {
      var tipus = new TipusPermis('Permís 01').perHores().noRecuperable();
      PermisManager.tipusPermisos = [];
      assert.deepStrictEqual(PermisManager.getTipusPermisPerNom('Permís 01'), tipus);
    });
  });
});